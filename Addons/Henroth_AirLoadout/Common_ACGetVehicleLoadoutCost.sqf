...
for [ {_mounted_index = 0},{ _mounted_index < ( count ( _chosen_loadout ))},{ _mounted_index = _mounted_index + 1}] do
{
	// Get the mount point mounted array
	_mounted_mount_array = _vehicle_mount_options select ( 1 + _mounted_index );
	...
	//player sideChat format [ "_mounted_options %1" , _chosen_loadout];
	...
	_mounted_mount = _chosen_loadout select _mounted_index;
	//player sideChat format [ "_mounted_mount %1" , _mounted_mount];
	//player sideChat format [ "_chosen_loadout %1" , _chosen_loadout];
	_weapon_chosen = _mounted_mount select ( ( _mounted_weapon_index * 2 ) + 1 );
	//player sideChat format [ "_weapon_chosen %1" , _weapon_chosen];\
	_weapon_name = _mounted_mount select (  _mounted_weapon_index * 2 );
