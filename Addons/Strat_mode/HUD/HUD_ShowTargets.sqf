...
//if ( (vehicle player) getVariable "AN_iNet" == CTI_P_SideID) then {
...

	{
		...
		if ((_x select 0) in _shared_targets) then {drawIcon3D ["a3\ui_f\data\map\Markers\NATO\group_7.paa", [_color select 0,_color select 1,_color select 2,1], _pos , _size*2, _size*2, 0,"", 0, 0, "PuristaMedium"];};
