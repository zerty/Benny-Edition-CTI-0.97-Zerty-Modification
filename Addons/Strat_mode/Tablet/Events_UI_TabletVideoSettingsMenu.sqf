...
switch (_action) do {
	case "onLoad": {
		...
		((uiNamespace getVariable "cti_dialog_ui_videosettingsmenu") displayCtrl 150010) ctrlSetText (if (profileNamespace getVariable ["HUD_Normal",true]) then {"Info hud: On"} else {"Info hud: Off"});
		((uiNamespace getVariable "cti_dialog_ui_videosettingsmenu") displayCtrl 150011) ctrlSetText (if (profileNamespace getVariable ["HUD_Tactical",true]) then {"Tactical hud: On"} else {"Tactical hud: Off"});
	};
	...
	case "onHudBasicPressed": {
		...
		((uiNamespace getVariable "cti_dialog_ui_videosettingsmenu") displayCtrl 150010) ctrlSetText (if (_changeto) then {"Info hud: On"} else {"Info hud: Off"});
		saveProfileNamespace;
	};
	case "onHudTacPressed": {
		...
		((uiNamespace getVariable "cti_dialog_ui_videosettingsmenu") displayCtrl 150011) ctrlSetText (if (_changeto) then {"Tactical hud: On"} else {"Tactical hud: Off"});
		saveProfileNamespace;
	};
