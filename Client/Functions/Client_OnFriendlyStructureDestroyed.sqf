...
if !(_sell) then {
	HUD_NOTIFICATIONS pushBack [format ["(%2) %1 Destroyed!", (_var select 0) select 1, mapGridPosition _position],time+30,"ffff00"];
} else {
	HUD_NOTIFICATIONS pushBack [ format ["(%2) %1 Sold!", (_var select 0) select 1, mapGridPosition _position],time+10,"ffffff"];
};
