...
if (_side_new == CTI_P_SideJoined) then { //--- The player's side has captured it
	...
	HUD_NOTIFICATIONS pushBack [ format["%1 has been captured!", _town getVariable "cti_town_name"],time+30,"ffffff"];

} else { //--- The player's side has lost it
	//todo move to displaymessage

	if !(CTI_P_SideJoined == resistance) then{ HUD_NOTIFICATIONS pushBack [ format["%1 has been lost!", _town getVariable "cti_town_name"],time+40,"ffff00"]};
};
