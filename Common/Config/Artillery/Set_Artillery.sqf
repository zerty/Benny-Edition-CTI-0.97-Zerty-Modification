...
for '_i' from 0 to count(_c)-1 do {
	...
	if (isClass (configFile >> 'CfgVehicles' >> _classname)) then {
		...
		_mags_labels = [];
		{[_mags_labels, getText(configFile >> "CfgMagazines" >> _x >> "displayName")] call CTI_CO_FNC_ArrayPush} forEach (_m select _i);
		...
		[_artillery, _classname] call CTI_CO_FNC_ArrayPush;
