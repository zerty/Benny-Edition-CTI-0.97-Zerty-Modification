...
_vehicle spawn {
	while { !isNull _this && alive _this && ! cti_gameover } do {
		    sleep 20;
		    if (!isNull _this && alive _this && !((position _this) inArea [[worldSize/2, worldSize/2, 6000], worldSize/2, worldSize/2, 0, true, 6500])) then {(driver _this) groupChat (localize "STR_Area_Warning");};
		    if (!isNull _this && alive _this && !((position _this) inArea [[worldSize/2, worldSize/2, 6000], (worldSize/2) + 2000, (worldSize/2) + 2000, 0, true, 8500])) then {_this setDamage 1};
