...
while {alive _vehicle && time - _timeout <= _delay} do {
	switch (CTI_VEHICLES_HANDLER_EMPTY) do { //--- Before
		case 0: {if ({alive _x} count crew _vehicle > 0 && !((_vehicle iskindof "UGV_01_base_F" || _vehicle iskindof "UAV_02_base_F") && !(isUAVConnected _vehicle) )) then {_timeout = time}};
		case 1: {if (({alive _x} count crew _vehicle > 0 || canMove _vehicle || canFire _vehicle)&& !((_vehicle iskindof "UGV_01_base_F" || _vehicle iskindof "UAV_02_base_F") && !(isUAVConnected _vehicle) ) ) then {_timeout = time}};
	};
